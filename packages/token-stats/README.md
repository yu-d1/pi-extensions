...
→ ● with-clock     5h: 89% ⏱ 4h 15m W: 72% ⏱ 2d
  ○ compact        5h: 89% W: 72%
  ○ nearest-clock  5h: 89% W: 72% ⏱ 4h 15m
  ○ with-clock-short     5h: 89% ⏱ 4h W: 72% ⏱ 2d
  ○ nearest-clock-short  5h: 89% W: 72% ⏱ 4h
```

默认使用 `with-clock`，5 小时和周额度分别显示自己的倒计时；`nearest-clock` 只显示两个重置时间中较近的一个；带 `-short` 的样式只显示最大时间单位，例如 `4h 15m` 显示为 `4h`，`15m` 仍显示为 `15m`。
...
其中“状态栏配置”进入显示样式、显示内容、查询间隔和联网搜索；“统计查询”进入今日、按小时、本周和月度统计。
...
- **v1.5.2** — 关闭套餐的 provider 自动模糊匹配，改为必须按当前 provider 手动选择；主菜单调整为状态栏配置、套餐配额配置、统计查询三级入口；新增配额显示样式（默认 `with-clock`，支持紧凑和最近重置时间）；5h/周额度显示项各自包含对应刷新倒计时，移除独立的"刷新时间"显示项；修复倒计时与 ⏱ 之间的多余空格
