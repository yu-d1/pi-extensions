...
- **登录后刷新**：`/login <供应商名称>` 成功输入 API 密钥后，只刷新当前供应商一次；本地删除或编辑模型不会触发远程刷新。
...
5. 在供应商的模型管理中可以再次手动刷新模型，或手动添加模型 id。
6. 如需调整模型上下文，进入“管理模型” → “修改上下文窗口”，可填写 `256k`、`512k`、`1m` 或纯数字。
7. 使用 `/model` 选择对应的 `供应商名称/模型 id`。
